...
The Electron UI test uses an isolated user-data directory, real preload/main IPC, and a local HTTP Ollama protocol fixture. It exercises connection failure, an empty model list, refreshed model availability, streamed structured output, malformed/interrupted/error responses, cancellation before headers and during a stream, and preserved work across navigation. Unit tests additionally cover split UTF-8/NDJSON records, request validation, renderer ownership and delayed events. This verifies transport and UI behavior without a local LLM or model downloads; real model quality and device capture require the services and permissions described above.

Ollama availability and server version are checked through `/api/version` and `/api/tags`; **Refresh models** retries the connection and refreshes the cached list. Model selection does not depend on GPU inventory. Downloaded model bytes do not predict runtime VRAM use, so the selector does not mark models as fitting or approaching GPU capacity. The default endpoint is `http://127.0.0.1:11434`; set `OLLAMA_HOST` before starting the app to use another endpoint (a host and port or full HTTP URL).

The [Whisper GPU evaluation](docs/whisper-gpu-evaluation.md) records native CPU/WebGPU comparisons, packaged-app checks, precision choices, and fallback experiments. Whisper continues to use CPU/q8; detecting a GPU or successfully opening a GPU session alone does not demonstrate faster transcription.

Summaries stream a readable draft, then show a validated summary, decisions, and action items with optional owners and due dates. The app supplies a JSON schema through [Ollama structured outputs](https://docs.ollama.com/capabilities/structured-outputs) and validates the complete response with Zod. A stream without the final completion record, invalid JSON, or missing fields fails explicitly; drafts never become completed or exported summaries. **Cancel generation** stops the individual HTTP request, including while waiting for the server. Starting another summary cancels the previous request. Navigating away from Home preserves the active meeting, transcript, and streaming summary; closing the window cancels outstanding work. Errors include a localized explanation and expandable technical details. Requests accept up to 1,000,000 transcript characters and time out after ten minutes; status checks time out after five seconds. Structured output is bounded to 100,000 summary characters, 200 decisions and 200 action items.
